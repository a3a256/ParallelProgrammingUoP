...
public class ParallelTracer extends Thread{
    ...
    public static void main(String [] args) throws Exception {
        ...
        ParallelTracer [] threads = new ParallelTracer [P] ;

        for(i=0; i<P; i++){
            threads[i] = new ParallelTracer(i, spheres);
            threads[i].start();
        }
        ...
        System.out.println("Calculations completed in " + (timeEnd - timeStart) + " milliseconds");
    }
    ...
    public ParallelTracer(int thread, ArrayList<Sphere> spheres){
        this.me = thread;
    ...
    public void run() {
        ...
        int step = IMAGE_HEIGHT/P;
        int y_start = me*step;
        int y_end = y_start+step;
        // Trace rays
        for (int y = y_start; y < y_end; ++y) {
            for (int x = 0; x < IMAGE_WIDTH; ++x) {
                ...
                // System.out.println("R: " + image[x][y].r+ " G: " + image[x][y].g + " B: " + image[x][y].b);
            }
            display.repaint() ;
        }
    }
